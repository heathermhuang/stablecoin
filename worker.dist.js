...
.chain-ico{width:16px;height:16px;border-radius:50%;overflow:hidden;flex-shrink:0;background:var(--bg-3);border:1px solid var(--border-1);display:flex;align-items:center;justify-content:center;font-size:7px;font-weight:700;color:var(--text-3)}
.chain-ico img{width:16px;height:16px;border-radius:50%;display:block}
.chain-more{font-size:9px;color:var(--text-3);font-family:'IBM Plex Mono',monospace;white-space:nowrap}
.ex-badges{display:flex;gap:3px;align-items:center;flex-wrap:wrap}
.ex-badge{display:inline-flex;align-items:center;justify-content:center;min-width:20px;height:18px;padding:0 4px;border-radius:3px;font-size:8px;font-weight:800;font-family:'IBM Plex Mono',monospace;letter-spacing:0;cursor:default}
.ex-badge.binance{background:rgba(240,185,11,0.15);color:#F0B90B;border:1px solid rgba(240,185,11,0.2)}
.ex-badge.coinbase{background:rgba(0,82,255,0.15);color:#6b9fff;border:1px solid rgba(0,82,255,0.2)}
.ex-badge.kraken{background:rgba(87,65,217,0.15);color:#a78bfa;border:1px solid rgba(87,65,217,0.2)}
.ex-badge.okx{background:rgba(255,255,255,0.08);color:var(--text-2);border:1px solid var(--border-2)}
.ex-badge.bybit{background:rgba(247,166,0,0.15);color:#fbbf24;border:1px solid rgba(247,166,0,0.2)}
.ex-badge.bitfinex{background:rgba(22,177,87,0.15);color:#4ade80;border:1px solid rgba(22,177,87,0.2)}
.ex-badge.kucoin{background:rgba(0,218,154,0.15);color:#34d399;border:1px solid rgba(0,218,154,0.2)}
.ex-badge.bitget{background:rgba(0,200,255,0.12);color:#22d3ee;border:1px solid rgba(0,200,255,0.18)}
.ex-badge.gate{background:rgba(227,6,19,0.12);color:#f87171;border:1px solid rgba(227,6,19,0.18)}
.ex-badge.upbit{background:rgba(0,44,134,0.2);color:#60a5fa;border:1px solid rgba(0,44,134,0.25)}
...
function chainIconsHtml(chains, max = 6) {
  ...
  const icons = show.map(ch => {
    ...
    const url = \`https://icons.llamao.fi/icons/chains/rsz_\${slug}.jpg\`;
    const initials = ch.slice(0,2).toUpperCase();
    return \`<span class="chain-ico" title="\${ch}"><img src="\${url}" alt="\${ch}" onerror="this.parentNode.textContent='\${initials}'"></span>\`;
  }).join('');
  const more = extra > 0 ? \`<span class="chain-more">+\${extra}</span>\` : '';
  return \`<div class="chain-icons">\${icons}\${more}</div>\`;
}

// Exchange badge rendering
const EX_LABELS = {
  binance:'B', coinbase:'CB', kraken:'K', okx:'OKX',
  bybit:'BY', bitfinex:'BFX', kucoin:'KC', bitget:'BG',
  gate:'GT', upbit:'UB',
};

function exchangeBadgesHtml(exchanges) {
  ...
  return \`<div class="ex-badges">\${exchanges.map(ex =>
    \`<span class="ex-badge \${ex}" title="\${ex.charAt(0).toUpperCase()+ex.slice(1)}">\${EX_LABELS[ex]||ex.toUpperCase().slice(0,3)}</span>\`
  ).join('')}</div>\`;
}
...
function renderTable() {
  ...
  tbody.innerHTML = page.map(c => {
    ...
    return \`<tr>
      ...
      <td class="col-chains">\${chainIconsHtml(c.chains, 6)}</td>
      <td class="col-exchanges">\${exchangeBadgesHtml(c.exchanges)}</td>
